...
async def worker(lock, name, delay):
    ...
    async with lock as tid:
        print(f"[{time.strftime('%H:%M:%S')}] 任务 {name} 启动, 领到 ID: {tid}, 预计耗时 {delay}s")
...
async def main():
    ...
    print(f"[{time.strftime('%H:%M:%S')}] 【观察者 A】醒了！耗时 {end_a - start_a:.2f}s (预期约 2s)")
    ...
    print(f"[{time.strftime('%H:%M:%S')}] 【观察者 B】醒了！耗时 {end_b - start_b:.2f}s (预期约 4s)")
